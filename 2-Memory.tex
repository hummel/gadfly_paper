...
While use of a refinement criterion minimizes unnecessary I/O operations, this approach is fairly rigid, and on its own would fail when attempting to load additional fields directly into an existing \code{PartType} dataframe from which particles have been dropped by the user.
To mitigate this, \code{gadfly} separately maintains a full list of particle IDs with which newly loaded fields are combined in a \code{pandas.Series} data structure.  
