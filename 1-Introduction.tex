...
\textsc{gadget} and its successor \textsc{gizmo} \citep{gizmo_paper} are widely used to investigate a range of astrophysical problems; unfortunately this also leads to fractionation of the data storage format as each research group modifies the output to suit its needs.
...
Recently, the scientific python community has begun to converge on the \code{DataFrame} provided by the high-performance \code{pandas} data analysis library as a common data structure for the ecosystem. 
...
In this paper we present the first public release (v0.1) of \code{gadfly}. 
The framework design and organizational structure are outlined in Section \ref{sec:framework}, followed by a description of the included SPH particle rendering (Section \ref{sec:vis}) and parallel batch processing (Section \ref{sec:parallel}) capabilities.  Our plans for future development are outlined in Section \ref{sec:future}, and a summary is provided in Section \ref{sec:summary}.
