...
W(r,h_{\rm sm}) =
     ...
     \end{cases}
\end{equation}
where $\bf r$ is the radius and $h_{\rm sm}$ is the characteristic width of the kernel, otherwise known as the smoothing length.  The physical density at any point, $\rho(\bf{r})$, is then represented by the sum over all particles
\begin{equation}
